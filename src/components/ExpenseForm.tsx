...
export default function ExpenseForm () {
  ...
  const handleSubmit = ( e: React.FormEvent<HTMLFormElement> ) => {
    ...
    //Que no me pase del limite del presupuesto
    //Lo que modifique - lo que tenia antes
    if( (expense.amount - previousAmount) > remainingBudget ){
      setError('Ese gasto supera el presupuesto')
      return
    }

    //agregar o editar el expense
    if(state.editingId){
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      <legend className="uppercase text-center text-2xl font-bold border 
        border-b-4 border-blue-500 py-2 rounded-md">
